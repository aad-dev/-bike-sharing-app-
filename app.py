...
st.write(
    "Vul de weersomstandigheden en datum in om te voorspellen hoeveel fietsen er dit uur verhuurd worden!"
)
...
weathersit = st.sidebar.selectbox("Weersituatie (1:Helder - 4:Noodweer)", [1, 2, 3, 4])

# de numerieke inputs
temp = st.sidebar.slider("Temperatuur (genormaliseerd 0-1)", 0.0, 1.0, 0.5)
atemp = st.sidebar.slider("Gevoelstemperatuur (genormaliseerd 0-1)", 0.0, 1.0, 0.5)
hum = st.sidebar.slider("Luchtvochtigheid (genormaliseerd 0-1)", 0.0, 1.0, 0.5)
windspeed = st.sidebar.slider("Windsnelheid (genormaliseerd 0-1)", 0.0, 1.0, 0.2)
